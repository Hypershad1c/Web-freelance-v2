...
const COLUMNS: Array<{ status: LeadStatus; label: string; tone: string }> = [
  { status: "NEW", label: "Nouveaux", tone: "border-sky-200 bg-sky-50" },
  { status: "CONTACTED", label: "Contactés", tone: "border-violet-200 bg-violet-50" },
  { status: "QUALIFIED", label: "Qualifiés", tone: "border-amber-200 bg-amber-50" },
  { status: "CONVERTED", label: "Convertis", tone: "border-emerald-200 bg-emerald-50" },
  { status: "LOST", label: "Perdus", tone: "border-stone-200 bg-stone-50" },
];
      ...
      <div className="grid min-w-[1100px] grid-cols-5 gap-4">
        {COLUMNS.map((column) => (
          <section
            ...
            className={`min-h-[30rem] rounded-2xl border p-3 ${column.tone}`}
          >
            <div className="mb-3 flex items-center justify-between px-1">
              <h2 className="font-display text-base font-semibold text-domify-dark">{column.label}</h2>
              <span className="flex h-6 min-w-6 items-center justify-center rounded-full bg-white px-1.5 text-xs font-bold text-domify-dark/65">{byStatus[column.status].length}</span>
            </div>
            <div className="space-y-3">
              {byStatus[column.status].map((lead) => (
                <article
                  ...
                  className="cursor-grab rounded-xl border border-black/7 bg-white p-3 shadow-[0_10px_22px_-18px_rgba(16,47,66,0.5)] transition-shadow hover:shadow-md active:cursor-grabbing"
                >
