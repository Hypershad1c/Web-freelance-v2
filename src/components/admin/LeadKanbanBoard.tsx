...
import { CheckCircle2, Clock3, Columns3, Eye, GripVertical, List, LoaderCircle, Mail, MapPin, MessageSquareText, Phone, RotateCcw, Trash2, X } from "lucide-react";
...
type LeadCard = {
  ...
  delivery?: { accepted: boolean; createdAt: Date } | null;
};
...
export function LeadKanbanBoard({ leads, canOpenProperty, canDelete, canResend }: { leads: LeadCard[]; canOpenProperty: boolean; canDelete: boolean; canResend: boolean }) {
  ...
  async function resendLeadNotification(lead: LeadCard) {
    setResendingLeadId(lead.id);
    try {
      const response = await fetch(`/api/admin/leads/${lead.id}/resend-notification`, { method: "POST" });
      const body = await response.json() as { error?: string; recipientCount?: number };
      if (!response.ok) throw new Error(body.error || "Le fournisseur email n’a pas accepté l’envoi.");
      setItems((previous) => previous.map((item) => item.id === lead.id ? { ...item, delivery: { accepted: true, createdAt: new Date() } } : item));
      window.alert(`Notification acceptée par le fournisseur pour ${body.recipientCount ?? 0} destinataire(s).`);
      router.refresh();
    } catch (error) {
      window.alert(error instanceof Error ? error.message : "Le renvoi de notification a échoué.");
    } finally {
  ...
  return (
    <div className="relative">
      ...
      {selectedLead && <LeadDetailPanel lead={selectedLead} canOpenProperty={canOpenProperty} canDelete={canDelete} canResend={canResend} busy={isPending || deletingLeadId === selectedLead.id || resendingLeadId === selectedLead.id} resending={resendingLeadId === selectedLead.id} onClose={() => setSelectedLeadId(null)} onStatusChange={(status) => moveLead(selectedLead.id, status)} onDelete={() => void deleteLead(selectedLead)} onResend={() => void resendLeadNotification(selectedLead)} />}
    </div>
  );
}
