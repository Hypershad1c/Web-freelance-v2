...
import { CheckCircle2, Clock3, Columns3, Eye, GripVertical, List, LoaderCircle, Mail, MapPin, MessageSquareText, Phone, X } from "lucide-react";
...
export function LeadKanbanBoard({ leads, canOpenProperty }: { leads: LeadCard[]; canOpenProperty: boolean }) {
  ...
  return (
    <div className="relative">
      ...
      {selectedLead && <LeadDetailPanel lead={selectedLead} canOpenProperty={canOpenProperty} busy={isPending} onClose={() => setSelectedLeadId(null)} onStatusChange={(status) => moveLead(selectedLead.id, status)} />}
    </div>
  );
}
