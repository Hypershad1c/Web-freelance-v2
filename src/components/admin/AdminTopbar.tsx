...
export async function AdminTopbar({ title }: { title: string }) {
  ...
  return (
    <header className="admin-safe-top relative flex min-h-16 flex-wrap items-center justify-between gap-3 border-b border-domify-dark/7 bg-[#fcfbf8]/92 px-4 py-3 shadow-[0_12px_30px_-28px_rgba(16,47,66,0.6)] backdrop-blur-xl sm:flex-nowrap sm:py-2 lg:min-h-20 lg:px-10">
      ...
      <div className="ml-auto flex shrink-0 items-center gap-1.5 sm:ml-4 sm:gap-4"><span className="hidden rounded-full border border-domify-gold/20 bg-domify-warm-white px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.12em] text-domify-primary md:inline-flex">Live</span>
