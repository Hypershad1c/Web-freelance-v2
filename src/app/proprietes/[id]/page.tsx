...
import { Bed, Bath, Square, MapPin, Calendar, Hash, Check, Heart, Scale } from "lucide-react";
import { getPropertyById, getSimilarProperties, properties } from "@/lib/mock-data";
        ...
        <div>
          ...
          <div className="mt-8 flex flex-col justify-between gap-4 border-b border-black/5 pb-8 sm:flex-row sm:items-start">
            ...
            <div className="flex items-center gap-3">
              <button className="flex h-10 w-10 items-center justify-center rounded-full border border-black/10 text-domify-dark/70 hover:border-domify-gold hover:text-domify-gold transition-luxury" aria-label="Ajouter aux favoris">
                <Heart size={17} />
              </button>
              <button className="flex h-10 w-10 items-center justify-center rounded-full border border-black/10 text-domify-dark/70 hover:border-domify-primary hover:text-domify-primary transition-luxury" aria-label="Comparer">
                <Scale size={17} />
              </button>
              <p className="text-2xl font-bold text-domify-gold whitespace-nowrap">
                {formatMAD(property.price)}
                {property.type === "location" && <span className="text-sm font-normal text-domify-dark/50"> /mois</span>}
              </p>
            </div>
          </div>
          ...
          <div className="mt-8 grid grid-cols-2 gap-6 sm:grid-cols-4">
            ...
            <Fact icon={Square} label="Surface" value={`${property.area} m²`} />
          ...
          <div className="mt-10">
            ...
            <p className="mt-3 leading-relaxed text-domify-dark/70">{property.description}</p>
          ...
          <div className="mt-10">
            ...
            <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3">
              {property.amenities.map((a) => (
                <span key={a} className="flex items-center gap-2 rounded-lg bg-domify-warm-white px-3 py-2.5 text-sm text-domify-dark/80">
                  <Check size={14} className="text-domify-gold" /> {a}
                </span>
              ))}
            </div>
          </div>
          ...
          <div className="mt-10">
            ...
            <div className="mt-4 flex h-64 items-center justify-center rounded-2xl bg-domify-warm-white text-sm text-domify-dark/50">
              Carte interactive (Leaflet) — {property.neighborhood}, {property.city}
            </div>
          </div>
        </div>
