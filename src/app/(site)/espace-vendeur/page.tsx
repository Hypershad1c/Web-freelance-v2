...
import { ArrowUpRight, BarChart3, CalendarClock, ClipboardList, FileText, Handshake, Home, Shield, Sparkles } from "lucide-react";
...
export default async function SellerPortalPage() {
  ...
  const submittedProperties = await prisma.property.findMany({
    ...
    select: { id: true, title: true, titleEn: true, titleAr: true, description: true, descriptionEn: true, descriptionAr: true, reference: true, price: true, surfaceArea: true, bedrooms: true, bathrooms: true, address: true, listingType: true, approvalStatus: true, status: true, rejectionReason: true, submittedAt: true, updatedAt: true, city: { select: { name: true } }, neighborhood: { select: { name: true } }, propertyType: { select: { name: true } }, _count: { select: { media: true, amenities: true } } },
      ...
      <div className="mx-auto max-w-7xl px-4 py-9 sm:px-6 lg:px-8">
        {!contact && submittedProperties.length === 0 ? (
          ...
          <>
            ...
            <section className="mt-8 rounded-[1.65rem] border border-domify-gold/20 bg-domify-warm-white/70 p-5 sm:p-7"><div className="flex items-start justify-between gap-4"><div><p className="luxury-eyebrow text-domify-gold">Confiance & vérification</p><h2 className="mt-2 font-display text-2xl font-semibold text-domify-dark">Votre dossier de confiance</h2><p className="mt-2 text-sm leading-6 text-domify-dark/55">Domify vérifie les documents nécessaires et le statut de vos annonces avant publication.</p></div><Shield className="text-domify-gold" size={21} /></div><div className="mt-5 grid gap-3 sm:grid-cols-3"><VerificationItem label="Compte identifié" complete={Boolean(session.user.email)} /><VerificationItem label="Documents vérifiés" complete={sellerDocuments.some((document) => document.status === "VERIFIED")} /><VerificationItem label="Bien approuvé" complete={submittedProperties.some((property) => property.approvalStatus === "APPROVED")} /></div></section>
            ...
            <section className="mt-8 rounded-[1.65rem] border border-domify-dark/8 bg-white/95 p-5 shadow-[0_22px_48px_-34px_rgba(16,47,66,0.42)] sm:p-7"><div className="flex items-center justify-between gap-4"><div><p className="luxury-eyebrow text-domify-gold">Vos dépôts</p><h2 className="mt-2 font-display text-2xl font-semibold text-domify-dark">Annonces en vérification</h2><p className="mt-2 text-xs leading-5 text-domify-dark/55">Le score de préparation vous indique les éléments à compléter pour renforcer la confiance et la visibilité de votre annonce.</p></div><Link href="/vendre-louer" className="inline-flex shrink-0 items-center gap-2 rounded-xl bg-domify-primary px-3 py-2 text-xs font-semibold text-white hover:bg-domify-primary-dark"><Home size={14} /> Déposer</Link></div><div className="mt-6 grid gap-3 md:grid-cols-2">{submittedProperties.length === 0 ? <p className="rounded-2xl bg-domify-warm-white p-5 text-sm leading-6 text-domify-dark/60">Vos annonces déposées apparaîtront ici avec leur statut de vérification.</p> : submittedProperties.map((property) => { const approval = APPROVAL_STAGES[property.approvalStatus] || APPROVAL_STAGES.DRAFT; const quality = assessListingQuality({ ...property, cityName: property.city.name, neighborhoodName: property.neighborhood?.name, mediaCount: property._count.media, amenityCount: property._count.amenities }); const qualityTone = quality.level === "ready" ? "bg-emerald-50 text-emerald-700" : quality.level === "improve" ? "bg-amber-50 text-amber-700" : "bg-red-50 text-red-700"; return <article key={property.id} className="rounded-2xl bg-domify-warm-white/70 p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate font-semibold text-domify-dark">{property.title}</p><p className="mt-1 text-xs text-domify-dark/55">{property.reference} · {property.city.name} · {property.propertyType.name}</p></div><span className={`shrink-0 rounded-full px-2.5 py-1 text-[0.68rem] font-semibold ${approval.tone}`}>{approval.label}</span></div><div className="mt-4 flex flex-wrap gap-x-4 gap-y-1 text-xs text-domify-dark/55"><span>{property.listingType === "VENTE" ? "Vente" : "Location"}</span><span>{formatMAD(property.price)}</span><span>{property.surfaceArea} m²</span></div><div className="mt-4 rounded-xl bg-white/85 p-3"><div className="flex items-center justify-between gap-3"><p className="text-xs font-semibold text-domify-dark">Préparation de l’annonce</p><span className={`rounded-full px-2 py-1 text-[0.65rem] font-bold ${qualityTone}`}>{quality.score}/100</span></div><div className="mt-2 h-1.5 overflow-hidden rounded-full bg-domify-dark/8"><div className={`h-full rounded-full ${quality.level === "ready" ? "bg-emerald-500" : quality.level === "improve" ? "bg-amber-400" : "bg-red-400"}`} style={{ width: `${quality.score}%` }} /></div>{quality.missing.length > 0 && <p className="mt-2 text-xs leading-5 text-domify-dark/55">À améliorer : {quality.missing.slice(0, 3).join(" · ")}</p>}</div>{property.rejectionReason && <p className="mt-3 rounded-xl bg-red-50 px-3 py-2 text-xs leading-5 text-red-700">{property.rejectionReason}</p>}<p className="mt-3 text-[0.68rem] text-domify-dark/45">Soumis le {property.submittedAt ? new Intl.DateTimeFormat("fr-MA", { dateStyle: "medium" }).format(property.submittedAt) : "—"}</p></article>; })}</div></section>

            <section className="mt-8 rounded-[1.65rem] border border-domify-dark/8 bg-white/95 p-5 shadow-[0_22px_48px_-34px_rgba(16,47,66,0.42)] sm:p-7"><div className="flex items-center justify-between gap-4"><div><p className="luxury-eyebrow text-domify-gold">Visibilité</p><h2 className="mt-2 font-display text-2xl font-semibold text-domify-dark">Performance de vos biens</h2></div><div className="flex items-center gap-3"><Link href="/espace-vendeur/statistiques" className="inline-flex items-center gap-1 rounded-lg border border-domify-primary/15 px-3 py-2 text-xs font-semibold text-domify-primary hover:border-domify-gold/50 hover:text-domify-gold">Analyse détaillée <ArrowUpRight size={14} /></Link><FileText className="text-domify-gold" size={21} /></div></div><div className="mt-6 grid gap-4 md:grid-cols-2">{properties.length === 0 ? <p className="text-sm text-domify-dark/60">Vos biens apparaîtront ici dès qu&apos;ils seront rattachés à votre dossier et publiés.</p> : properties.map((property) => <article key={property.id} className="rounded-2xl bg-domify-warm-white/70 p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-semibold text-domify-dark">{property.title}</p><p className="mt-1 text-xs text-domify-dark/55">{property.reference} · {property.status}</p></div><Link href={`/proprietes/${property.id}`} className="rounded-lg bg-white p-2 text-domify-primary shadow-sm hover:text-domify-gold" aria-label={`Voir ${property.title}`}><ArrowUpRight size={15} /></Link></div><div className="mt-5 grid grid-cols-3 gap-2 text-center text-xs"><Metric label="Vues" value={property.viewsCount} /><Metric label="Favoris" value={property._count.favorites} /><Metric label="Demandes" value={property._count.leads + property._count.appointments} /></div></article>)}</div></section>
          </>
        )}
      </div>
    </main>
...
function VerificationItem({ label, complete }: { label: string; complete: boolean }) { return <div className="rounded-xl bg-white/80 p-4"><div className={`h-2 w-2 rounded-full ${complete ? "bg-emerald-500" : "bg-amber-400"}`} /><p className="mt-3 text-sm font-semibold text-domify-dark">{label}</p><p className="mt-1 text-xs text-domify-dark/50">{complete ? "Confirmé" : "À compléter"}</p></div>; }
