      ...
      <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
        {!contact || contact.sellerCases.length === 0 ? (
          ...
          <>
            ...
            <section className="mt-8 rounded-[1.45rem] border border-domify-dark/8 bg-white p-5 shadow-[0_20px_42px_-34px_rgba(16,47,66,0.45)] sm:p-6"><div className="flex items-center justify-between"><div><p className="luxury-eyebrow text-domify-gold">Rapports</p><h2 className="mt-2 font-display text-2xl font-semibold text-domify-dark">Suivi hebdomadaire</h2></div><BarChart3 className="text-domify-gold" size={21}/></div><div className="mt-5 grid gap-3 md:grid-cols-2">{contact.ownerReports.length === 0 ? <p className="rounded-2xl bg-domify-warm-white p-5 text-sm leading-6 text-domify-dark/60">Votre premier point hebdomadaire apparaîtra ici après la publication ou l’activation de votre dossier.</p> : contact.ownerReports.map((report) => <article key={report.id} className="rounded-2xl bg-domify-warm-white/70 p-4"><p className="font-semibold text-domify-dark">Point du {new Intl.DateTimeFormat("fr-MA", { dateStyle: "medium" }).format(report.periodStart)} au {new Intl.DateTimeFormat("fr-MA", { dateStyle: "medium" }).format(report.periodEnd)}</p><p className="mt-2 text-xs text-domify-dark/55">{report.status === "SENT" ? `Envoyé le ${report.deliveredAt ? new Intl.DateTimeFormat("fr-MA", { dateStyle: "medium" }).format(report.deliveredAt) : ""}` : report.status === "SKIPPED" ? "Préparé, en attente de consentement email" : "Préparé"}</p></article>)}</div></section>

            <section className="mt-8 rounded-[1.45rem] border border-domify-dark/8 bg-white p-5 shadow-[0_20px_42px_-34px_rgba(16,47,66,0.45)] sm:p-6"><div className="flex items-center justify-between"><div><p className="luxury-eyebrow text-domify-gold">Coffre documentaire</p><h2 className="mt-2 font-display text-2xl font-semibold text-domify-dark">Vos documents</h2></div><FileText className="text-domify-gold" size={21}/></div><div className="mt-6 grid gap-3 md:grid-cols-2">{contact.documents.length === 0 ? <p className="rounded-2xl bg-domify-warm-white p-5 text-sm leading-6 text-domify-dark/60">Les documents que votre conseiller partage avec vous apparaîtront ici. Les documents demandés sont signalés pour vous aider à préparer votre dossier.</p> : contact.documents.map((document) => <article key={document.id} className="rounded-2xl bg-domify-warm-white/70 p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-semibold text-domify-dark">{document.name}</p><p className="mt-1 text-xs text-domify-dark/55">{document.type.replaceAll("_", " ")} · {new Intl.DateTimeFormat("fr-MA", { dateStyle: "medium" }).format(document.createdAt)}</p></div><span className="rounded-full bg-white px-2.5 py-1 text-[0.68rem] font-semibold text-domify-primary">{document.status === "REQUESTED" ? "À fournir" : document.status === "VERIFIED" ? "Vérifié" : document.status === "REJECTED" ? "À corriger" : "Disponible"}</span></div>{document.rejectedReason && <p className="mt-3 text-xs leading-5 text-red-700">{document.rejectedReason}</p>}{document.status !== "REQUESTED" && document.url !== "https://domify.ma/document-request" && <a href={document.url} target="_blank" rel="noreferrer" className="mt-4 inline-flex rounded-lg bg-white px-3 py-2 text-xs font-semibold text-domify-primary shadow-sm hover:text-domify-gold">Ouvrir le document</a>}</article>)}</div></section>
