  ...
  return (
    <main className="mx-auto max-w-7xl bg-[radial-gradient(circle_at_85%_0%,rgba(232,203,145,0.12),transparent_24rem)] px-4 py-9 sm:px-6 lg:px-8 lg:py-14">
      ...
      <div className="mt-10 grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
        <section id="alertes" className="rounded-[1.65rem] border border-domify-dark/8 bg-white/95 p-5 shadow-[0_22px_48px_-34px_rgba(16,47,66,0.4)] sm:p-7"><div className="flex items-start justify-between gap-4"><div><p className="luxury-eyebrow text-domify-gold">Ne manquez rien</p><h2 className="mt-2 font-display text-2xl font-semibold text-domify-dark">Mes alertes de recherche</h2><p className="mt-2 text-sm leading-6 text-domify-dark/55">Recevez les nouvelles opportunités correspondant à vos critères.</p></div><BellRing size={20} className="mt-1 text-domify-primary/40" /></div><SavedSearchForm cities={cities} propertyTypes={propertyTypes}/>{savedSearches.length > 0 && <div className="mt-6 grid gap-2 sm:grid-cols-2">{savedSearches.map((search) => <div key={search.id} className="rounded-xl border border-domify-dark/7 bg-domify-warm-white/60 px-4 py-3"><div className="flex items-center justify-between gap-2"><b className="text-sm text-domify-dark">{search.name}</b><div className="flex items-center gap-1"><span className={`h-2 w-2 rounded-full ${search.active ? "bg-emerald-500" : "bg-domify-dark/25"}`} /><SavedSearchActions id={search.id} active={search.active} /></div></div><p className="mt-1 text-xs text-domify-dark/55">{search.city?.name || "Toutes villes"} · {search.propertyType?.name || "Tous types"} · {search.channel} · {search.active ? "Active" : "En pause"}</p></div>)}</div>}</section>
